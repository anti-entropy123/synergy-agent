	...
	for {
		...
		case x := <-in:
			if jobs[x.Id] == 0 && x.Credit == -3 { //任务状态是未执行
				...
				new_x := PidI{x.Pid, x.Job, x.N1, x.Id, x.St, init_credit, x.t0}
				...
				new_x := PidI{x.Pid, x.Job, x.N1, x.Id, time.Now(), cur_credit, x.t0}
				ts_chan <- new_x
				queue <- new_x
			} else {
				jobs[x.Id] = 0
				...
				tcpuCacheLock.Lock()
				utime := sfsTcpuCache[x.Job]
				tcpuCacheLock.Unlock()

				fmt.Println("User CPU Time ", utime, x.Job)
				// fmt.Printf("receive t1=%v\n", t1)
				// if exist {
				t2 := time.Now()
				...
				// }
				fmt.Println(x.Job, x.t0[11:], t1.Format("15:04:05.000"), t2.Format("15:04:05.000"), t1.Sub(t0).Milliseconds(), t2.Sub(t1).Milliseconds(), t2.Sub(t0).Milliseconds())
		...
		for k, v := range jobs { //k、v是jobs数组中的索引和值,k任务id,v任务状态
			if v == 2 { //v == 2是任务休眠状态
				...
				if GetProcessState(pids[k]) == 1 { //从休眠被唤醒
					...
					panic("will gen wrong t0")

					var fibString string = fmt.Sprintf("fib%d", k)
	...
	for {
		...
		case x, _ := <-in:
			...
			if on == 0 { //on为0表示队列处于关闭状态
				...
				new_pid := PidI{-1, "minus", q.UpdateValue, -1, time.Now(), x.Credit, x.t0}
				cfs_chan <- new_pid
				on = 1
			}
