...
# 不同的进程名称
processes1 = ['f5', 'f4', 'f3', 'f2', 'f1']
processes2 = ['f5', 'f4', 'f3', 'f2', 'f1']
processes3 = ['f5', 'f3', 'f1', 'f2', 'f4']
...
for ax, data, title, processes in zip(axes, datasets, titles, processes_lists):
    ...
    for i, (process, wait, fifo, cfs) in enumerate(zip(processes, wait_times, fifo_times, cfs_times)):
        ax.barh(process, wait, color=colors[0], edgecolor='black', height=bar_width, label=labels[0] if i == 0 else "")
        ax.barh(process, fifo, left=wait, color=colors[1], edgecolor='black', height=bar_width, label=labels[1] if i == 0 else "")
        ax.barh(process, cfs, left=wait+fifo, color=colors[2], edgecolor='black', height=bar_width, label=labels[2] if i == 0 else "")

        # 添加文字标签
        if wait > 3:
            ax.text(wait / 2, process, str(wait), va='center', ha='center', color='black')
        if fifo > 3:
            ax.text(wait + fifo / 2, process, str(fifo), va='center', ha='center', color='black')
        if cfs > 3:
            ax.text(wait + fifo + cfs / 2, process, str(cfs), va='center', ha='center', color='white')
...
ax3.annotate(
    text='1',           # 标注文本
    xy=(1, 4.1),           # 箭头指向的点 (数据坐标)
    xytext=(-30, 4.4),             # 文本位置 (数据坐标)
    arrowprops=dict(
...
# 调整布局
# plt.tight_layout()
plt.subplots_adjust(wspace=0.15, hspace=0.4, left=0.05, right=0.97, top=0.92, bottom=0.1)
