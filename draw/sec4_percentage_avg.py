...
for ratio in range(50, 101, 10):
    ...
    hw_sgf_avg.append(hw_sgf_trace[:int(ratio*length)].mean())
    ...
    wr_sgf_avg.append(wr_sgf_trace[:int(ratio*length)].mean())
    ...
    print("百分位:", ratio)
    print("hw:", hw_sg_avg[-1], hw_of_avg[-1], hw_ow_avg[-1], hw_sfs_avg[-1], hw_sgf_avg[-1])
    print("wr:", wr_sg_avg[-1], wr_of_avg[-1], wr_ow_avg[-1], wr_sfs_avg[-1], wr_sgf_avg[-1])
...
# 绘制柱状图
axes[0].bar(index - 2*bar_width, hw_sg_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="Synergy")
axes[0].bar(index - 1*bar_width, hw_of_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="OpenFaaS")
axes[0].bar(index, hw_ow_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="OpenWhisk")
axes[0].bar(index + 1*bar_width, hw_sfs_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="SFS")
axes[0].bar(index + 2*bar_width, hw_sgf_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="Synergy-P")
...
axes[0].legend(loc=(0., 1.02), ncol=5, frameon=False, handlelength=1, handletextpad=0.5, columnspacing=1.5)

# 绘制柱状图
# 设置柱状图的边框宽度
axes[1].bar(index - 2*bar_width, wr_sg_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="Synergy")
axes[1].bar(index - 1*bar_width, wr_of_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="OpenFaaS")
axes[1].bar(index, wr_ow_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="OpenWhisk")
axes[1].bar(index + 1*bar_width, wr_sfs_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="SFS")
axes[1].bar(index + 2*bar_width, wr_sgf_avg, bar_width, linewidth=line_width, zorder=10, edgecolor="black", label="Synergy-P")
...
plt.savefig("pdf/sec4_percentage_avg.pdf")